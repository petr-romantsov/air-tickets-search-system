...
import { InputEvent } from '../Components/Options/OptionsWrapper/OptionsWrapper.types';
...
export const useFlightsList = () => {
  const [flights] = useState<Flight[]>(allFlights);
  ...
  const uniqueAirlines = new Set(
    flights.map(flight => flight.flight.carrier.caption),
  );
  const airlines = Array.from(uniqueAirlines);

  const getMinPrices = (flights: Flight[]) => {
  ...
  const handleSorting: InputEvent = event => {
    let sortedFlights: Flight[] = [...filteredFlights].sort((a, b) => {
  ...
  const handlePriceFiltration = (min: string, max: string) => {
    ...
    if (min) {
      filtered = filtered.filter(
        flight => Number(flight.flight.price.total.amount) >= Number(min),
      );
    }

    if (max) {
      filtered = filtered.filter(
        flight => Number(flight.flight.price.total.amount) <= Number(max),
      );
    }
    setFilteredFlights(filtered);
  };
    ...
    let filtered =
      selectedAirlines.length > 0
        ? flights.filter(flight =>
            selectedAirlines.includes(flight.flight.carrier.caption),
          )
        : flights;
